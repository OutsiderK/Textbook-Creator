...
Trigger: `inspect_state.py` reports `kp_status.pool` non-empty, or user says "重平衡 / 做 rebalance / 看看现在能写哪些".

> **Stage B 的座右铭：定形。** 这是**唯一**做章节级决策的阶段。它不写正文。它的输出是"谁进哪章"+"哪些走 patch"+"哪些先留着"。
...
每个被处理的 pool KP **必须**得到一个明确归属：

| 归属 | 在 KP 上的表现 |
|---|---|
| 进入新章 | `status: queued`, `applied_to: chNN-slug`, `reader_notice: none`, `action: new_chapter` |
| 补入已有章 | `status: queued`, `applied_to: chNN-slug`, `reader_notice: needed`, `action: patch_chapter` |
| 暂留 pool | `status: pool` 不变, **必须**填 `pool_reason` 字段说明等待理由（语义判断，下文有规则） |
...
batch: 003
target: pool-cluster
kp_ids: [<all pool KPs being considered>]
notes: "Stage B rebalancing pool of N KPs after ingest batch 003"
```

Recovery: rebalance writes to KP yaml atomically (one file rewrite). If interrupted before that write, just re-run. If interrupted after, the `.in-progress.yaml` is stale and should be cleared by user choosing "discard".
...
如果某个 KP 的 `prerequisites` 全在某已写章里，且不构成新主题 → patch 该旧章。
如果某个 KP 的 `extends` 指向某已写章的 KP → patch。
如果 `applied_to` 已暗示某旧章（来自 PPT 反向引用的早期内容）→ patch。

Patch 候选的 KP 不进新章簇。

### Q6 — 是否有 PPT 细节应进入例题/方法/常见误区，而不是正文主线？

这是关于 detail_cards 的使用，不影响 KP 的章节归属。
做好标记即可（不必单独建状态）。Stage C/D 会读 detail_cards 决定放主线还是 sidebar/exercise。
...
如果 KP 已在 pool 超过 2 个 batch 而没动 → **强制在 audit 里讨论**，提示用户："`OSPPT-CH08-MMF` 在 pool 中已停留 3 批，pool_reason: enrichment_only。是否手动指定归属？"

## Action 与 reader_notice 表

| 簇决策 | 每个 KP 设置 |
|---|---|
| 形成新章 chNN | `status: queued`, `action: new_chapter`, `applied_to: chNN-slug`, `reader_notice: none` |
| 补入已有章 chMM | `status: queued`, `action: patch_chapter`, `applied_to: chMM-slug`, `reader_notice: needed` |
| 暂留 | `status: pool`, `pool_reason: <one of above>` |

**注意**：`action` 字段在 KP 进 `status: queued` 时强制存在；当 KP 流转到 `status: applied` 后，`action` 字段可保留作历史，但读取时无效。
...
```
重平衡 batch 003 完成：
  新章建议: ch08-memory-management (5 个 KP)
  补入旧章: ch07-storage-foundations (2 个 KP，需生成 supplement)
  暂留 pool: 1 个 KP (OSPPT-CH08-MMF, reason: bridging_undefined)
