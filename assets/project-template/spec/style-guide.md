...
| 情况 | 优先使用 |
|---|---|
| "A 不是 B，而是 C" 中的 B/C 对比焦点 | `<u>` |
| "当 value < 0 时" 这类短条件或短限制 | `<u>` |
| "只在多 CPU / 多生产者场景下" 这类范围标记 | `<u>` |
| "互斥是同步的特殊情形" 这类核心判断 | `==` 或 callout |
| "value 为负表示等待队列长度" 这类应试断言 | `==` 或 callout |
| 首次定义"记录型信号量" | `**` |
| 三步以上流程、比较、状态关系 | 图、表、公式或步骤 |

如果一章有大量 `==...==` 但没有 `<u>...</u>`，要检查是否把局部条件、范围或对比焦点误写成了高亮。
